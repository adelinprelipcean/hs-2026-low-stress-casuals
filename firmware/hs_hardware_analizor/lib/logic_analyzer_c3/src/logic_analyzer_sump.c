...
// Different SUMP clients encode read-count differently (N+1 vs (N+1)*4).
// Pick the valid interpretation for current HW limits.
static int decode_read_count(uint32_t raw_count)
{
    logic_analyzer_hw_param_t hw = {
        .current_channels = LA_ANALYZER_CHANNELS,
        .current_psram = LA_ANALYZER_PSRAM,
    };
    logic_analyzer_get_hw_param(&hw);

    int count_x1 = (raw_count < 0x7fffffffU) ? ((int)raw_count + 1) : hw.max_sample_cnt;
    int count_x4 = (raw_count < 0x1fffffffU) ? (((int)raw_count + 1) * 4) : hw.max_sample_cnt;

    int x1_valid = (count_x1 >= hw.min_sample_cnt && count_x1 <= hw.max_sample_cnt);
    int x4_valid = (count_x4 >= hw.min_sample_cnt && count_x4 <= hw.max_sample_cnt);
    ...
    if (count_x4 > hw.max_sample_cnt && count_x1 <= hw.max_sample_cnt)
    {
        return clamp_int(count_x1, hw.min_sample_cnt, hw.max_sample_cnt);
    }
    return clamp_int(count_x4, hw.min_sample_cnt, hw.max_sample_cnt);
}
...
{
    logic_analyzer_hw_param_t hw = {
        .current_channels = la_cfg.number_channels,
        .current_psram = la_cfg.samples_to_psram,
    };
    logic_analyzer_get_hw_param(&hw);

    int requested_samples = (readCount > 0) ? readCount : la_cfg.number_of_samples;
    int requested_rate = PULSEVIEW_MAX_SAMPLE_RATE / (divider + 1);

    la_cfg.number_of_samples = clamp_int(requested_samples, hw.min_sample_cnt, hw.max_sample_cnt);
    la_cfg.sample_rate = clamp_int(requested_rate, hw.min_sample_rate, hw.max_sample_rate);
    readCount = la_cfg.number_of_samples;
...
static void sump_cmd_parser(uint8_t cmdByte)
{
    ...
    case SUMP_SET_READ_DELAY_COUNT: // samples or bytes ??????
        sump_getCmd4(cmd.u_cmd8);
        readCount = decode_read_count(cmd.u_cmd16[0] & 0xffff);
        delayCount = decode_read_count(cmd.u_cmd16[1] & 0xffff);
        break;
    case SUMP_SET_BIG_READ_CNT: // samples or bytes ??????
        sump_getCmd4(cmd.u_cmd8);
        readCount = decode_read_count(cmd.u_cmd32);
...
static void sump_get_metadata()
{
    ...
    sump_writeByte((uint8_t)la_hw.current_channels);
    /* protocol version (2) */
